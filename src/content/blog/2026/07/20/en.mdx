...
This is the loose end from [#13](/en/blog/swift-zero-expert-protocols): a protocol with an `associatedtype` is a *generic protocol*, and you can't always use it as a plain `any Container` — the box wouldn't know what `Item` is. The clean way to hand one back is the opaque `some` keyword — which is exactly what [#15](/en/blog/swift-zero-expert-opaque-types) covers next. Associated types are the reason `some` exists.
</Callout>
...
- **Associated type** — `associatedtype Item` makes a protocol generic; conformers fill it in (often inferred); the reason `some` ([#15](/en/blog/swift-zero-expert-opaque-types)) exists
