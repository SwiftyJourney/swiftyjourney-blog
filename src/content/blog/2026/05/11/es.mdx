...
El `for-in` es la forma más natural de recorrer secuencias en Swift — arrays, diccionarios, rangos, strings, cualquier cosa iterable (técnicamente, cualquier tipo que conforme el protocolo `Sequence` — más sobre protocolos en un artículo posterior).
...
(`throws` y `try?` son la sintaxis de manejo de errores de Swift — la veremos en un artículo posterior; aquí concéntrate en cómo `defer` se ejecuta en cada ruta de salida.)
...
![Orden LIFO de defer: los defers A, B, C se registran de arriba a abajo y se ejecutan en orden inverso — C, B, A — al salir del scope](./resources/defer-lifo.png)
...
![Jump table frente a cadena if/else: el panel izquierdo comprueba los valores 0, 1, 2, 3 uno por uno hasta encontrar la coincidencia (O(n)); el panel derecho indexa table[3] directamente al slot correspondiente en un único acceso (O(1))](./resources/jump-table.png)
