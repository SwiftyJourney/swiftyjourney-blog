...
Paso a paso, así es como las variables capturadas se mueven del stack a una capture box en el heap:

![Diagrama de cinco estados: un stack frame con runningTotal y amount; al retornar el closure se mueven a una capture box en el heap (refCount 1); las llamadas repetidas mutan runningTotal en esa box; un segundo closure obtiene su propia box independiente; asignar el closure comparte la misma box y sube su refCount a 2](./resources/closure-capture.png)
...
Un closure **escapa** (escapes) una función cuando sobrevive más allá de la ejecución de esa función. Esto sucede cuando el closure se guarda en una variable externa, se almacena en un array, o se ejecuta de forma asíncrona después de que la función retorna. (El código asíncrono — código que termina más tarde, como una respuesta de red — se cubre en el artículo #16.)
</Callout>
...
En un escaping closure dentro de una clase, debes escribir `self` explícitamente al acceder a propiedades. Esto es intencional — Swift te obliga a ser consciente de que estás capturando `self` — cómo una instancia se refiere a sí misma desde dentro de sus propios métodos — porque es fácil crear **retain cycles** (referencias circulares que nunca se liberan). Profundizaremos en esto en el artículo #16 sobre ARC.
...
Una referencia *weak* no mantiene vivo al objeto — si todo lo demás lo libera, el objeto se destruye y la referencia weak se vuelve `nil` automáticamente (por eso `self` es opcional dentro del closure). Una referencia normal es *strong*: mantiene vivo al objeto mientras la referencia exista.
</Callout>
...
- `[unowned self]` → Cuando estás seguro de que self vivirá más que el closure. Más rápido que weak (menos trabajo de registro para el runtime), pero crashea si te equivocas.
...
// Ahora reescrito con @autoclosure (reemplaza la versión de arriba —
// no declares ambas en el mismo scope o tendrás un error de redeclaración).
...
![Anatomía de un closure: una estructura Closure con dos slots — un function pointer que apunta a código compartido de solo lectura en el segmento __TEXT, y un capture context que apunta a una capture box en el heap con runningTotal y amount y un refCount](./resources/closure-anatomy.png)
...
- **Non-escaping closure sin capturas** → Zero cost, puede vivir en registros (registros del CPU — el almacenamiento más rápido)
- **Non-escaping closure con capturas** → puede alocarse en el stack (típicamente zero heap cost)
