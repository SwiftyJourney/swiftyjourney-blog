...
Step by step, here is how the captured variables move from the stack to a capture box on the heap:

![Five-state diagram: a stack frame holding runningTotal and amount; the closure returned moves them to a capture box on the heap (refCount 1); repeated calls mutate runningTotal in that box; a second closure gets its own independent box; assigning the closure shares the same box and raises its refCount to 2](./resources/closure-capture.png)
...
A closure **escapes** a function when it survives beyond that function's execution. This happens when the closure is saved to an external variable, stored in an array, or executed asynchronously after the function returns. (Asynchronous code — code that finishes later, like a network response — is covered in article #16.)
</Callout>
...
In an escaping closure inside a class, you must write `self` explicitly when accessing properties. This is intentional — Swift forces you to be aware that you're capturing `self` — how an instance refers to itself from inside its own methods — because it's easy to create **retain cycles** (circular references that never get freed). We'll dive deep into this in article #16 on ARC.
...
A *weak* reference doesn't keep the object alive — if everything else releases it, the object is freed and the weak reference automatically becomes `nil` (which is why `self` is optional inside the closure). A normal reference is *strong*: it keeps the object alive as long as the reference exists.
</Callout>
...
- `[unowned self]` → When you're certain self will outlive the closure. Faster than weak (less bookkeeping for the runtime), but crashes if you're wrong.
...
// Now rewritten with @autoclosure (replaces the version above —
// don't declare both in the same scope or you'll get a redeclaration error).
...
![Anatomy of a closure: a Closure struct with two slots — a function pointer that points to shared read-only code in the __TEXT segment, and a capture context that points to a capture box on the heap holding runningTotal and amount with a refCount](./resources/closure-anatomy.png)
...
- **Non-escaping closure without captures** → Zero cost, can live in registers (CPU registers — fastest storage)
- **Non-escaping closure with captures** → can be stack-allocated (typically zero heap cost)
