...
![Dos paneles: el call site greet(person: "Bill", from: "Cupertino") con person y from marcados como argument labels, y el cuerpo de la función usando person y hometown como parameter names](./resources/argument-labels.png)
...
La semántica oficial de `inout` es **copy-in copy-out**: el valor se copia al entrar a la función y se copia de vuelta al salir. Pero el compilador optimiza esto a **pass-by-reference** siempre que puede — especialmente cuando la variable original es local y no hay aliasing (ningún otro nombre apuntando a la misma variable al mismo tiempo). Resultado: la semántica de valor se mantiene (los value types como `Int` o un struct se copian al pasarlos; semántica de valor significa que la variable de quien llama se comporta como si hubiera recibido su propia copia independiente), pero el rendimiento es el de una referencia.
</Callout>

![Lado a lado: el modelo copy-in/copy-out (el valor se copia al entrar, se muta y se copia de vuelta al salir) frente a la ruta optimizada pass-by-reference (la dirección se pasa directamente y se muta en su lugar sin copias)](./resources/inout-copy-vs-reference.png)
...
![Una variable mathFunction con tipo (Int, Int) -> Int donde la etiqueta del tipo permanece fija mientras addTwoInts y multiplyTwoInts se intercambian como la función asignada](./resources/function-types-swap.png)
...
`printMathResult` no sabe ni le importa qué hace `operation` — solo sabe que acepta dos `Int` y retorna un `Int`. Esto es **polimorfismo** (el mismo código funcionando con muchos comportamientos distintos) **a través de tipos de función** — uno de los pilares de la programación funcional.
...
![Diagrama de segmentos de memoria con tres regiones: __TEXT para el código de la función de solo lectura y compartido, el stack frame para los parámetros value type, las variables locales y el puntero de función de 8 bytes, y el closure context en el heap creado solo cuando una función captura variables](./resources/function-memory-segments.png)
