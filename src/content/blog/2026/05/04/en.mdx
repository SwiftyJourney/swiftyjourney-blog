...
// 👨 + ZWJ + 👩 + ZWJ + 👧 + ZWJ + 👦
// ZWJ (Zero-Width Joiner) is an invisible Unicode scalar (U+200D)
// that tells the text engine to fuse the surrounding emojis into one glyph.
```

![Diagram showing the family emoji as a chain of 7 Unicode scalars (👨 U+1F468, ZWJ U+200D, 👩 U+1F469, ZWJ, 👧 U+1F467, ZWJ, 👦 U+1F466) collapsing into a single Character box where family.count equals 1](./resources/grapheme-cluster-zwj.png)
...
Both `String` and `Substring` conform to `StringProtocol`. If you write functions that accept text, use `some StringProtocol` as the parameter type to accept both without forcing a copy. (`some` means "some concrete type that conforms" — opaque types are covered later in the series.)
</Callout>
...
![Diagram aligning the string "Dog‼🐶" against its three encodings in parallel rows: UTF-8 bytes (10 total), UTF-16 code units (6 total), and Unicode scalar values (5 total), showing how the wider characters expand into more code units](./resources/unicode-encodings-triple.png)
...
![Diagram comparing two String values: on the left "Hello" (5 bytes) stored inline inside the 16-byte struct with no heap arrow, on the right a 100-byte string whose struct holds a pointer to a heap buffer](./resources/small-string-optimization.png)
...
- `index(after:)` → **O(1)** per grapheme cluster
