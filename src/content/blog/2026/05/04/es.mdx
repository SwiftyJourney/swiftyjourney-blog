...
// 👨 + ZWJ + 👩 + ZWJ + 👧 + ZWJ + 👦
// ZWJ (Zero-Width Joiner) es un Unicode scalar invisible (U+200D)
// que le dice al motor de texto que fusione los emojis vecinos en un solo glifo.
```

![Diagrama que muestra el emoji de familia como una cadena de 7 Unicode scalars (👨 U+1F468, ZWJ U+200D, 👩 U+1F469, ZWJ, 👧 U+1F467, ZWJ, 👦 U+1F466) que colapsa en una única caja Character donde family.count es igual a 1](./resources/grapheme-cluster-zwj.png)
...
Tanto `String` como `Substring` conforman `StringProtocol`. Si escribes funciones que aceptan texto, usa `some StringProtocol` como parámetro para aceptar ambos sin forzar una copia. (`some` significa "algún tipo concreto que conforma" — los opaque types se cubren más adelante en la serie.)
</Callout>
...
![Diagrama que alinea el string "Dog‼🐶" contra sus tres codificaciones en filas paralelas: bytes UTF-8 (10 en total), code units UTF-16 (6 en total) y valores de Unicode scalar (5 en total), mostrando cómo los caracteres más anchos se expanden en más code units](./resources/unicode-encodings-triple.png)
...
![Diagrama que compara dos valores String: a la izquierda "Hello" (5 bytes) almacenado inline dentro del struct de 16 bytes sin flecha al heap, a la derecha un string de 100 bytes cuyo struct guarda un pointer a un buffer en el heap](./resources/small-string-optimization.png)
...
- `index(after:)` → **O(1)** por grapheme cluster
...
- **Value Type con CoW** — struct value-type; buffer de caracteres en el heap (a menos que el string sea lo bastante corto para Small String Optimization — entonces los bytes viven inline en el struct), copy-on-write
