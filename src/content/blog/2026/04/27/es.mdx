...
numbers.append(1)
print(numbers.capacity) // crece (el valor exacto depende de la implementación)

numbers.append(2)
numbers.append(3)
print(numbers.capacity) // crece exponencialmente — podrías ver números distintos
```

Los valores exactos de capacity son un detalle de implementación de la librería estándar y del toolchain, no una garantía — lo que sí se promete es que la capacity crece de forma *exponencial*, que es lo que hace que `append` sea O(1) amortizado.
...
// Forma corta — elige una de las dos (Set se debe declarar, el tipo se infiere)
...
`remove` retorna un Optional — una caja que o contiene un valor o es `nil`. `if let removed = ...` lo desempaqueta solo cuando hay un valor. Cobertura completa en el artículo #11.
...
![Diagrama de Venn de las cuatro operaciones de Set: union, intersection, subtracting y symmetricDifference, con A = oddDigits y B = primes, mostrando cada resultado como una región sombreada](./resources/set-operations-venn.png)
...
// Con literal (tipo inferido como [String: String])
...
![Diagrama de copy-on-write en cuatro pasos: (1) crear el array, refcount 1; (2) asignar una copia, refcount 2 compartiendo un buffer; (3) mutar mientras está compartido, lo que copia primero para que cada variable tenga su propio buffer; (4) mutar cuando refcount es 1, lo que muta el buffer en el sitio sin copiar](./resources/copy-on-write.png)
...
Aquí `filter` conserva solo los elementos que cumplen una condición, y `{ $0 > 0 }` es un closure — una mini-función en línea donde `$0` es cada elemento. (Cubrimos closures en detalle en el artículo #6.)
...
El compilador de Swift puede incluso eliminar la comprobación de refCount. Si analiza tu código y demuestra que solo hay una referencia al buffer (por ejemplo, una variable local que nunca se comparte), el optimizador puede omitir la comprobación de unicidad y generar código que muta directamente — la misma idea detrás de `isKnownUniquelyReferenced(_:)`.
</Callout>
...
El struct en el stack es diminuto — en realidad solo un puntero al buffer en el heap (de hecho `MemoryLayout<[Int]>.size` es 8). El `count` y la `capacity` viven en el header del buffer en el heap, junto al `refCount` que Swift usa para decidir si necesita copiar.
...
Porque el compilador necesita saber el tamaño exacto de cada tipo en el stack en tiempo de compilación. Un `Array` puede tener 3 elementos o 3 millones — ese tamaño dinámico solo puede vivir en el heap. El struct en el stack siempre tiene el mismo tamaño fijo: un solo puntero (8 bytes). El `count` y la `capacity` viajan en el header del buffer en el heap.
</Callout>
