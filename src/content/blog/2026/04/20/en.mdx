...
What happens in memory when we call `calculateArea`?

1. **A stack frame is created** for the function. The stack pointer advances.
2. The parameters `base` (8 bytes) and `height` (8 bytes) are copied into the frame.
3. The local variable `area` (8 bytes) is computed and stored in the frame.
4. The return value is copied to the calling context.
5. **The stack frame is destroyed**. The stack pointer moves back. All clean.
