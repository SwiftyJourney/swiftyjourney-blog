...
When you use `let`, you're not just communicating your intent to the team — you're giving the **compiler information**. You're telling it: "this value will never change." And the compiler uses that promise to make optimization decisions. It can store the value directly in a processor register (a tiny slot of ultra-fast storage inside the CPU itself), eliminate unnecessary checks, or even substitute references to the value with the value itself (constant folding — the compiler computes the value once at build time and bakes it straight into the code).
...
Strings deserve their own article — and they'll get one in #3 of this series. For now, what you need to know is that `String` is a **value type** in Swift (it's a `struct`). A value type means each variable holds its own independent copy of the data — not a shared link to it. The opposite, reference types, we'll meet when we get to classes. But its actual content — the characters — is stored in a buffer that may live on the heap — a flexible but slower memory area we'll explain near the end of this article. We'll come back to this.
...
Another important detail: Swift's arithmetic operators **detect overflow by default**. If you try to add two `Int8` values that exceed their range, your app crashes with a clear message — in both debug and release builds. A deterministic crash beats a silently wrong result. If you intentionally need wrapping behavior (like C), use the special `&+` / `&-` / `&*` operators.
...
`user!` force-unwraps the optional (asserts it is not nil) — it crashes if it is, which is why the nil check guards it. Full optionals coverage is in #11.
...
Note: range subscripts yield an `ArraySlice<String>`, not a new `Array`; wrap with `Array(names[..<2])` if you need a fresh array.
...
Tuples are compound **value types**. Their size in memory is the sum of their components' sizes. The tuple `(Int, Bool)` takes up 9 bytes: 8 from the `Int` + 1 from the `Bool` (alignment padding — extra unused bytes the compiler sometimes inserts so values sit at memory addresses the CPU can read efficiently). Strictly speaking, 9 bytes is the logical `size`; the actual storage `stride` is 16 bytes due to 8-byte alignment.
...
If you've already read [Mastering Instruments (Part 2)](/blog/dominando-instruments-stack-heap-simbolizacion), you'll remember that the Stack is that fast, predictable chunk of memory where each function creates a "frame" on entry and destroys it on exit. No `malloc`/`free` (the manual memory-allocate/free calls from C), no ARC — just a pointer (a value holding a memory address) moving up and down.
...
- When it's stored in an **existential container** (a kind of box for protocol values — article #13)
...
When you write code that operates with value types on the stack, you're writing code the compiler can optimize aggressively. No reference counting, no synchronization, no indirection (concepts we will unpack later). It's the kind of code that makes Swift fast — and it's the kind of code you'll write naturally if you understand these fundamentals.
</Callout>
...
Think about the cost: zero at runtime. The compiler does all its work before generating the binary. When your app runs on the user's device, there are no type checks, no inference. All of that already happened. What's left is clean, optimized code that knows exactly what types it's working with.
