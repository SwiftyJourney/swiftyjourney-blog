...
Cuando usas `let`, no solo estás comunicando tu intención al equipo — estás dándole **información al compilador**. Le estás diciendo: "este valor no va a cambiar jamás". Y el compilador usa esa promesa para tomar decisiones de optimización. Puede almacenar ese valor directamente en un registro del procesador (una pequeñísima ranura de almacenamiento ultrarrápido dentro de la propia CPU), puede eliminar comprobaciones innecesarias, puede incluso sustituir las referencias al valor por el valor mismo (constant folding — el compilador calcula el valor una sola vez en tiempo de compilación y lo incrusta directamente en el código).
...
Usa type annotations cuando el tipo no sea obvio a simple vista, cuando quieras ser explícito para legibilidad, o cuando necesites un tipo diferente al que el compilador inferiría por defecto (por ejemplo, `let price: Float = 9.99` en lugar de `Double`).
</Callout>
...
Los strings merecen su propio artículo — y lo tendrán en el #3 de esta serie. Por ahora, lo que necesitas saber es que `String` es un **tipo de valor** en Swift (es un `struct`). Un tipo de valor significa que cada variable guarda su propia copia independiente de los datos — no un enlace compartido a ellos. Lo opuesto, los tipos de referencia, los conoceremos cuando lleguemos a las clases. Pero su contenido real — los caracteres — se almacena en un buffer que puede vivir en el heap — una zona de memoria flexible pero más lenta que explicaremos cerca del final de este artículo. Volveremos a esto.
...
Otro detalle importante: los operadores aritméticos de Swift **detectan overflow por defecto**. Si intentas sumar dos `Int8` que exceden su rango, tu app crashea con un mensaje claro — tanto en builds de debug como de release. Un crash determinista es mejor que un resultado silenciosamente incorrecto. Si necesitas intencionalmente el comportamiento de wrapping (como en C), usa los operadores especiales `&+` / `&-` / `&*`.
...
`user!` hace force-unwrap del opcional (afirma que no es nil) — crashea si lo es, por eso la comprobación de nil lo protege. La cobertura completa de opcionales está en el #11.
...
Nota: los subscripts de rango devuelven un `ArraySlice<String>`, no un nuevo `Array`; envuélvelo con `Array(names[..<2])` si necesitas un array fresco.
...
Las tuplas son **tipos de valor** compuestos. Su tamaño en memoria es la suma del tamaño de sus componentes. La tupla `(Int, Bool)` ocupa 9 bytes: 8 del `Int` + 1 del `Bool` (padding de alineación — bytes extra sin usar que el compilador a veces inserta para que los valores queden en direcciones de memoria que la CPU puede leer eficientemente). Estrictamente, 9 bytes es el `size` lógico; el `stride` real de almacenamiento es 16 bytes por la alineación de 8 bytes.
...
Si ya leíste [Dominando Instruments (Parte 2)](/blog/dominando-instruments-stack-heap-simbolizacion), recordarás que el Stack es esa pila de memoria rápida y predecible donde cada función crea un "frame" al entrar y lo destruye al salir. No hay `malloc`/`free` (las llamadas manuales de C para reservar/liberar memoria), no hay ARC — solo un puntero (un valor que contiene una dirección de memoria) que sube y baja.
...
- Cuando se almacena en un **contenedor existencial** (una especie de caja para valores de protocolo — artículo #13)
...
Cuando escribes código que opera con value types en el stack, estás escribiendo código que el compilador puede optimizar agresivamente. No hay conteo de referencias, no hay sincronización, no hay indirección (conceptos que desglosaremos más adelante). Es el tipo de código que hace que Swift sea rápido — y es el tipo de código que vas a escribir naturalmente si entiendes estos fundamentos.
</Callout>
...
Piensa en el costo: cero en runtime. El compilador hace todo su trabajo antes de generar el binario. Cuando tu app corre en el dispositivo del usuario, no hay verificaciones de tipo, no hay inferencia. Todo eso ya pasó. Lo que queda es código limpio, optimizado, que sabe exactamente con qué tipos está trabajando.
