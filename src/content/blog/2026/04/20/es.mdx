...
let resultado = calcularArea(base: 10.0, altura: 5.0)
```

¿Qué pasa en memoria cuando llamamos a `calcularArea`?

1. **Se crea un stack frame** para la función. El stack pointer avanza.
2. Los parámetros `base` (8 bytes) y `altura` (8 bytes) se copian al frame.
3. La variable local `area` (8 bytes) se calcula y se almacena en el frame.
4. El valor de retorno se copia al contexto que llamó la función.
5. **El stack frame se destruye**. El stack pointer retrocede. Todo limpio.
