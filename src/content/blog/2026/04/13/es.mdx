...
En la primera parte de esta serie aprendimos a usar Instruments como herramienta. Hoy aprendimos la anatomía del paciente. Pero hay una pieza que falta: mencionamos `malloc`, `free`, `refCount` — conceptos que suenan abstractos si nunca has trabajado a ese nivel. En el siguiente artículo vamos a cerrar esa brecha con **visualizaciones interactivas** que te dejan *ver* cómo funciona la memoria paso a paso.

<Callout type="success" title="Lo que viene en la Parte 2.5">
Un artículo interactivo donde vas a visualizar malloc reservando bloques, free liberándolos, ARC contando referencias, y retain cycles atrapando memoria — todo en componentes que puedes manipular directamente. Nos vemos ahí.
</Callout>
