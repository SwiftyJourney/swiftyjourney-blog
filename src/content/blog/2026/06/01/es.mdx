...
Si vienes de C o Java, probablemente piensas en los enums como una lista de enteros con nombre. En Swift, un enum es un **sum type** — es decir, un valor que es exactamente UNA de varias formas posibles a la vez (north O south, un UPC O un QR), lo que en la teoría de tipos se conoce como un *tipo algebraico completo* — con métodos, computed properties, associated values de distintos tipos, conformance a protocolos, y hasta recursión. Son una de las herramientas más expresivas del lenguaje, y entender cómo el compilador los representa en memoria te va a cambiar la forma en que los diseñas.
...
Un **protocolo** es una lista de capacidades que un tipo puede declarar que soporta (cobertura completa en el artículo #13). Cuando un tipo adopta `CaseIterable`, el compilador escribe por ti el código requerido — eso es lo que significa **sintetizar** aquí. En concreto, le da al enum una propiedad `allCases` que retorna una colección con todos los valores posibles del enum.
</Callout>
...
![Diagrama de bytes que muestra que una instancia de Planet sigue ocupando 1 byte (el tag) mientras los raw values viven en una tabla de lookup separada de compilación](./resources/enum-raw-values-metadata.png)
...
Fíjate en el `where` — puedes filtrar dentro del mismo case para manejar subcasos. El `where` agrega una condición extra: el case solo coincide si su patrón encaja Y la prueba del `where` es verdadera; de lo contrario Swift sigue revisando los cases de abajo. La combinación de associated values + pattern matching + where clauses es increíblemente poderosa.
...
El compilador de Swift puede sintetizar automáticamente la *conformance* a varios protocolos para enums — *conformance* significa simplemente que el tipo cumple la lista de capacidades del protocolo, y aquí el compilador escribe ese código por ti:
...
![Diagrama de bytes de un enum de 4 casos: un solo byte con 2 bits usados para el tag y 6 bits sin usar](./resources/enum-tag-bits.png)
...
Estas cifras exactas no son garantías del lenguaje: en una plataforma de 64 bits, los toolchains actuales reportan 33/40 bytes aquí, pero los números son detalles de implementación que dependen de la plataforma y el toolchain. Cuando `Barcode` es `.qrCode`, usa 16 de los 32 bytes de payload — los otros 16 quedan sin usar. Es el precio de tener un tamaño fijo para el tipo.

![Diagrama de barras de bytes comparando los casos de Barcode: .upc llena el payload de 32 bytes, .qrCode usa 16 bytes y desperdicia 16, ambos más un tag de 1 byte para un total de 33 bytes](./resources/enum-associated-values-size.png)
...
![Diagrama de la spare bit optimization: Bool? cabe en 1 byte usando el patrón de bits 2 para .none, y String? usa el null pointer 0x0 como .none sin bytes extra](./resources/enum-optional-spare-bit.png)
...
![Diagrama de un enum indirect: un puntero al heap de 8 bytes almacenado inline apunta a una caja .addition en el heap que contiene los casos tag, left y right](./resources/enum-indirect-heap.png)
...
No te preocupes por la sintaxis `~` y `consuming` ahora — es una feature avanzada de ownership que verás más adelante.
...
`~Copyable` requiere Swift 5.9+, `@nonexhaustive` requiere Swift 6.2.3+ (un toolchain reciente de Swift 6.2), y `@c` requiere Swift 6.3. No necesitas usarlos hoy, pero saber que existen te prepara para cuando los necesites — especialmente si escribes librerías o código de sistemas.
</Callout>
