...
If you come from C or Java, you probably think of enums as a list of named integers. In Swift, an enum is a **sum type** — meaning a value that is exactly ONE of several possible shapes at a time (north OR south, a UPC OR a QR code), known in type theory as a *full algebraic type* — with methods, computed properties, associated values of different types, protocol conformance, and even recursion. They're one of the most expressive tools in the language, and understanding how the compiler represents them in memory will change how you design them.
...
A **protocol** is a checklist of capabilities a type can declare it supports (full coverage in article #13). When a type adopts `CaseIterable`, the compiler auto-writes the required code for you — that's what **synthesize** means here. Specifically, it gives the enum an `allCases` property that returns a collection with all possible enum values.
</Callout>
...
![Byte diagram showing a Planet instance stays 1 byte (the tag) while raw values live in a separate compile-time lookup table](./resources/enum-raw-values-metadata.png)
...
Notice the `where` — you can filter within the same case to handle subcases. `where` adds an extra condition: the case only matches if its pattern fits AND the `where` test is true; otherwise Swift keeps checking the cases below. The combination of associated values + pattern matching + where clauses is incredibly powerful.
...
The Swift compiler can automatically synthesize *conformance* to several protocols for enums — *conformance* simply means the type satisfies the protocol's checklist, and here the compiler writes that code for you:
...
![Byte diagram of a 4-case enum: a single byte with 2 bits used for the tag and 6 bits unused](./resources/enum-tag-bits.png)
...
These exact figures aren't language guarantees: on a 64-bit platform, current toolchains report 33/40 bytes here, but the numbers are platform- and toolchain-dependent implementation details. When `Barcode` is `.qrCode`, it uses 16 of the 32 payload bytes — the other 16 go unused. That's the price of having a fixed size for the type.

![Byte-bar diagram comparing Barcode cases: .upc fills the 32-byte payload, .qrCode uses 16 bytes and wastes 16, both plus a 1-byte tag for 33 bytes total](./resources/enum-associated-values-size.png)
...
![Diagram of the spare bit optimization: Bool? fits in 1 byte by using bit pattern 2 for .none, and String? uses the null pointer 0x0 as .none for zero extra bytes](./resources/enum-optional-spare-bit.png)
...
![Diagram of an indirect enum: an 8-byte heap pointer stored inline points to a heap-allocated .addition box holding tag, left and right cases](./resources/enum-indirect-heap.png)
...
Don't worry about the `~` and `consuming` syntax now — it's an advanced ownership feature you'll meet later.
...
`~Copyable` requires Swift 5.9+, `@nonexhaustive` requires Swift 6.2.3+ (a recent Swift 6.2 toolchain), and `@c` requires Swift 6.3. You don't need to use them today, but knowing they exist prepares you for when you need them — especially if you write libraries or systems code.
</Callout>
...
- [SE-0487: Nonexhaustive enums](https://github.com/swiftlang/swift-evolution/blob/main/proposals/0487-non-exhaustive-enums.md)
