...
You can override a failable `init?` with a **nonfailable** `init` (a subclass can be stricter and guarantee success), but **not** the other way around. There's also a rarely-used `init!` variant — you can safely ignore it for now; optionals are covered in [#11](/en/blog/swift-zero-expert-optionals).
</Callout>
