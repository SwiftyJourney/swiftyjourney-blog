...
- **Inheritance** — one class can build on another, automatically getting its properties and methods while adding or replacing some (full detail in [#10](/en/blog/swift-zero-expert-inheritance-initialization))
...
![Value vs reference semantics: with value semantics, variables a and b each hold their own independent copy of a struct, so changing b leaves a untouched; with reference semantics, variables x and y both point with arrows to the same class object, so changing one changes both](./resources/value-vs-reference.png)
...
![Stack vs heap memory layout: on the left a struct lives entirely inside the function's stack frame with its width and height fields inline; on the right a class keeps only an 8-byte pointer on the stack that arrows to a heap block holding metadata, refcount, and the width/height data allocated with malloc](./resources/stack-vs-heap.png)

<Callout type="tip" title="Why does this matter?">
`malloc` is **orders of magnitude slower in the worst case** than stack allocation. Not because it's a bad algorithm — it's that finding free space and coordinating across threads is inherently slower than moving a single pointer. For a single object you won't notice, but in a loop that creates thousands of objects, the difference is real. Exact numbers vary by hardware, allocator contention, and object size.
</Callout>
...
With **static dispatch**, the compiler knows **at compile time** exactly which function to call (compile time = while Xcode is building your app; runtime = later, while the app is actually running on the device). There's no indirection (an extra hop before reaching the code) at runtime — the CPU jumps directly to the code. Structs use static dispatch for all their methods.
</Callout>

<Callout type="info" title="Definition: Dynamic Dispatch">
With **dynamic dispatch**, the program consults a **vtable** (virtual table) at runtime to determine which function to call. A vtable is a per-class lookup table that maps each method name to its actual implementation. This is necessary when a class might be subclassed and the method overridden — a *subclass* is a class built on top of another one, and *overriding* means replacing an inherited method with its own version (full detail in [article #10](/en/blog/swift-zero-expert-inheritance-initialization)). It costs a small, typically negligible overhead per call (a vtable lookup plus an indirect jump).
</Callout>
...
![Static vs dynamic dispatch: on the left static dispatch goes directly from the call site to the function's code in a single jump; on the right dynamic dispatch routes the call site through a vtable lookup before jumping to the code, and marking the class final collapses it back to the direct path](./resources/static-vs-dynamic-dispatch.png)
...
![Copy-on-Write in two states: first, original and copy both point to one shared buffer with refcount = 2 and no data is copied; second, calling copy.append(6) forces a copy so original keeps buffer A and copy gets a brand-new buffer B with the appended element, each now at refcount = 1](./resources/copy-on-write.png)
...
You can implement CoW in your own structs using `isKnownUniquelyReferenced(&storage)` — pass the internal class-backed storage by inout; if it returns `true` the storage is unique and you can mutate in place; if it has more owners, you copy first. Standard library types like Array do exactly this.
</Callout>
...
When you store a value type as `any Protocol`, Swift uses an **existential container** — a fixed-size structure that can hold the value inline (if it fits in 3 words / 24 bytes — a *word* is one CPU-sized memory slot, 8 bytes on modern Apple devices) or a pointer to the heap (if it's larger). Since Swift 5.6, `any` makes existential types explicit (and Swift 6 requires it). Don't worry if this feels abstract — it's an edge case you rarely need to reason about directly.
</Callout>
