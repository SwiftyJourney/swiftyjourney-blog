...
![Timeline of a lazy property: the instance is created with importer still nil at zero cost, stays nil while untouched, and DataImporter() runs only on first access](./resources/lazy-init-timeline.png)
...
A thread is a separate line of execution — when two run at once they can touch the same property simultaneously. If two threads access a lazy property simultaneously for the first time, the property could be initialized **twice**. If you need thread-safe initialization, use `static` (the Swift runtime guarantees it's initialized exactly once, even across threads — via `swift_once`, the modern equivalent of the old Objective-C `dispatch_once`) or an explicit synchronization mechanism.
</Callout>
...
![Comparison of the four property storage kinds — stored, lazy stored, computed, observers — showing their relative byte footprint in the instance: stored costs N bytes, lazy stored N plus a 1-byte flag, computed and observers zero](./resources/property-storage-taxonomy.png)
...
Stored type properties (`static let/var`) are initialized **lazily and thread-safely** by default. The Swift runtime guarantees initialization happens exactly once, even across threads (via `swift_once` — the modern equivalent of the old Objective-C `dispatch_once`): the first time you access the property, it's initialized exactly once regardless of how many threads access it simultaneously. It's the cleanest singleton (one shared instance) pattern there is.
</Callout>
...
![Memory layout of UserProfile: stored fields id (8 bytes), name (16 bytes), age (8 bytes), and isActive (1 byte) occupy labeled blocks followed by 7 bytes of alignment padding, reaching size 33 and stride 40 with alignment 8, while computed properties sit outside the layout as zero-byte function arrows](./resources/property-memory-layout.png)
...
- **Type properties** — `static`, one shared copy, initialized exactly once even across threads (via `swift_once`)
