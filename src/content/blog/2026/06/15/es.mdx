...
![Línea de tiempo de una lazy property: la instancia se crea con importer aún en nil a costo cero, sigue en nil mientras no se toca, y DataImporter() se ejecuta solo en el primer acceso](./resources/lazy-init-timeline.png)
...
Un hilo (thread) es una línea de ejecución independiente — cuando dos corren a la vez pueden tocar la misma propiedad de forma simultánea. Si dos hilos acceden a una lazy property simultáneamente por primera vez, la propiedad podría inicializarse **dos veces**. Si necesitas inicialización thread-safe, usa `static` (el runtime de Swift garantiza que se inicializa exactamente una vez, incluso entre hilos — vía `swift_once`, el equivalente moderno del antiguo `dispatch_once` de Objective-C) o un mecanismo explícito de sincronización.
</Callout>
...
![Comparación de los cuatro tipos de almacenamiento de propiedades — stored, lazy stored, computed, observers — mostrando su huella relativa en bytes dentro de la instancia: stored cuesta N bytes, lazy stored N más 1 byte de flag, computed y observers cero](./resources/property-storage-taxonomy.png)
...
Las stored type properties (`static let/var`) se inicializan de forma **lazy y thread-safe** automáticamente. El runtime de Swift garantiza que la inicialización ocurre exactamente una vez, incluso entre hilos (vía `swift_once` — el equivalente moderno del antiguo `dispatch_once` de Objective-C): la primera vez que accedes a la propiedad, se inicializa una sola vez sin importar cuántos hilos accedan simultáneamente. Es el patrón singleton (una sola instancia compartida) más limpio que existe.
</Callout>
...
- **Type properties** — `static`, una copia compartida, inicializada exactamente una vez incluso entre hilos (vía `swift_once`)
